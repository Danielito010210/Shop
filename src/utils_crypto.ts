// Pure JS SHA-256 implementation and password policy helpers for Cubanos en Miami
...
export function sha256(str: string): string {
  ...
  const lengthInBits = utf8.length * 8;
  const wordCount = ((utf8.length + 8) >> 6) * 16 + 14;
  words[wordCount] = (lengthInBits / 0x100000000) | 0;
  words[wordCount + 1] = lengthInBits | 0;
...
export function validatePassword(password: string): { isValid: boolean; error?: string } {
  if (password.length < 8) {
    return { isValid: false, error: "La contraseña debe tener al menos 8 caracteres de longitud." };
  }
  ...
  if (!specialCharRegex.test(password)) {
    return { isValid: false, error: "La contraseña debe contener al menos un carácter especial (ej. *, ?, !, @, $, #, _, -)." };
  }
  return { isValid: true };
}
